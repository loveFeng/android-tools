...
#nets=`adb shell ls /sys/class/net/`
nets="wlan0"
inet_byte() {
    for i in $nets; do
        let "$i"_rx"$1"=`adb shell cat /sys/class/net/$i/statistics/rx_bytes`
        let "$i"_tx"$1"=`adb shell cat /sys/class/net/$i/statistics/tx_bytes`
        #eval echo '$'"$i"_rx"$1"
    done
}
eva() {
...
while true; do
    sleep 2
    clear
    awk 'BEGIN {print "interface\trxKB(下载)\ttxKB(上传)\tTotalKB\n==========================================";}'
    inet_byte 1
    sleep 1
    inet_byte 2
    for i in $nets; do
